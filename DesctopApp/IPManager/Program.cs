...
// Allow the test project to see internal methods
...
namespace IPManager
{
    public class Program
    {
        ...
        internal static void HandleWebMessage(PhotinoWindow window, DatabaseService db, string message)
        {
            ...
            {
                SendJson(window, new { success = false, error = $"System error: {ex.Message}" });
            }
        }
                ...
                case "ADD_ASN":
                    if (int.TryParse(request.value, out int asnId) && asnId >= 0 && asnId <= 65535)
                    {
                        if (db.AsnExists(asnId))
                            SendJson(window, new { success = false, error = "This ASN already exists in the database." });
                        else
                            ProcessAsn(window, db, asnId);
                    }
                    else
                        SendJson(window, new { success = false, error = "Please enter a valid AS number (0-65535)" });
                    break;
            ...
            Task.Run(async () => {
                try
                {
                    ...
                    string msg = (info.ranges == null || info.ranges.Count == 0)
                        ? $"AS{asnId} added without IPv4 prefixes."
                        : $"AS{asnId} successfully added.";

                    SendJson(window, new { success = true, page = "asn", data = db.GetAsnList(), message = msg });
                }
                catch (Exception ex)
                {
                    SendJson(window, new { success = false, error = $"Error while loading AS{asnId}: {ex.Message}" });
                }
            });
        }
            ...
            try
            {
                ...
                foreach (var raw in File.ReadLines(path))
                {
                    ...
                    if (!IPAddress.TryParse(line, out var ip) || ip.AddressFamily != AddressFamily.InterNetwork)
                        throw new Exception($"Invalid IPv4 at line {lineNo}: {line}");
        ...
        private static void ProcessFindAsn(PhotinoWindow window, DatabaseService db, string ipStr)
        {
            Task.Run(async () => {
                try
                {
                    ...
                    if (foundAsn <= 0) throw new Exception("ASN could not be determined.");
                    ...
                    SendJson(window, new { success = true, command = "REFRESH_AFTER_FIND", message = $"IP mapped to AS{foundAsn}" });
                }
        ...
        private static void ShowFileDialog(PhotinoWindow window)
        {
            var thread = new Thread(() => {
                using var dialog = new OpenFileDialog { Filter = "Text files (*.txt)|*.txt", Title = "Select IP list" };
